...
class ValidateChecks:
	...
	def __init__(self, filled_df, parameters, get_params):
		...
		self.parameters = parameters
		self.global_logs = []
		self.sample_logs = defaultdict(list)  # sample_name -> list of messages
	...
	def validate_main(self):
		...
		self.check_custom_fields(parameters['custom_fields_file'])
	...
	def report_errors(self):
		with open(f'{self.parameters["output_dir"]}/{self.parameters["file_name"]}/errors/full_error.txt', "w") as f:
			...
			if self.global_logs:
				for line in self.global_logs:
					f.write(f"\t{line}\n")
			else:
				f.write("\tPassed all global checks!\n")

			f.write("\nSample Errors:\n\n")
			valid_samples = sum(1 for s in self.sample_logs if not any("ERROR" in msg for msg in self.sample_logs[s]))
			f.write(f"\tNumber of Valid Samples: {valid_samples}/{len(self.metadata_df)}\n\n")

			# Write Sample Errors
			for sample in self.metadata_df["sample_name"]:
				f.write(f"\t{sample}:\n")
				if sample in self.sample_logs and self.sample_logs[sample]:
					for log in self.sample_logs[sample]:
						f.write(f"\t\t{log.strip()}\n")
				else:
	...
	def check_date(self):
		...
		def validate_and_format(row):
			...
			if not date_str or date_str.strip() == "":
				self.sample_logs[sample].append("ERROR: Missing collection_date.")
				return date_str

			match = re.match(r"^(\d{4})(?:[-/](\d{1,2}))?(?:[-/](\d{1,2}))?", date_str)
			if not match:
				self.sample_logs[sample].append(f"ERROR: Invalid date format: '{date_str}'")
				return date_str

			year, month, day = match.groups()
			if len(year) == 2:
				self.sample_logs[sample].append(f"ERROR: Year is two digits: '{year}'")
				return date_str
	...
	def check_authors(self):
		...
		if 'authors' not in self.metadata_df.columns:
			self.global_logs.append("No 'authors' column found in metadata.")
			return
		...
		for idx, row in self.metadata_df.iterrows():
			...
			if fixed_authors_str != row.get('authors', ''):
				...
				self.sample_logs[sample_name].append("Author names were cleaned and reformatted.")
			...
			if field in self.metadata_df.columns:
				...
				if missing_mask.any():
					...
					for sample_name in self.metadata_df.loc[missing_mask, 'sample_name']:
						self.sample_logs[sample_name].append(f"WARNING: {field} is missing for sample {sample_name}, setting to 'Not Provided'\n")
			else:
				...
				if group_missing.any():
					for field in group_in_df:
						...
						for sample_name in self.metadata_df.loc[empty_field_mask, 'sample_name']:
							self.sample_logs[sample_name].append(
								f"WARNING: {field} in optional group is missing for sample {sample_name}, "
								f"setting to 'Not Provided'\n")
		# Final check
		if missing_fields:
			self.global_logs.append("Missing required fields: " + ", ".join(missing_fields))
		if missing_optionals:
			opt_str = ["[" + ", ".join(group) + "]" for group in missing_optionals]
			self.global_logs.append("Missing optional field groups: " + ", ".join(opt_str))
	...
	def check_meta_case(self):
		...
		try:
			...
			self.global_logs.append(f"Unexpected error during case metadata check: {str(e)}")
...
class HandleDfInserts:
	...
	"""
	def __init__(self, parameters, metadata_df):
		self.parameters = parameters
