...
class MainVADRFuncs:
    ...
    def split_table(self): 
        ...
        for i in range(len(indices)): 
            ...
            try:
                assert len([x for x in self.sample_info[key] if 'repeat_region' in x]) == 2
            except AssertionError:
                raise AssertionError(f"Found either less than or greater than two repeat regions in {key}")
