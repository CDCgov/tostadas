...
def metadata_validation_main():
	...
	# DataFrame.map() replaced applymap() in pandas 2.1; support both
	_map_fn = getattr(final_df, 'map', final_df.applymap)
	final_df = _map_fn(validate_checks.normalize_value)
...
class GetParams:
	...
	def get_args():
		...
		parser.add_argument("--use_llm", action="store_true", default=False,
							help="Use LLM (OpenAI) to generate plain-English error suggestions alongside error.txt. "
...
class ValidateChecks:
	...
	def validate_main(self):
		...
		# write error file
		self.report_errors()
		# optional LLM enhancement (no-op when use_llm=False or OPENAI_API_KEY absent)
		if self.parameters.get('use_llm', False):
			self._llm_enhance_errors()
	...
	def _llm_enhance_errors(self):
		"""Write an LLM-enhanced error report alongside error.txt when use_llm is enabled."""
		try:
			from llm_helper import explain_validation_errors
			with open('error.txt', 'r') as f:
				error_content = f.read()
			model = self.parameters.get('llm_model', 'gpt-4o-mini')
			enhanced = explain_validation_errors(error_content, model=model)
			if enhanced:
				with open('error_llm_suggestions.txt', 'w') as f:
					f.write("=== LLM-ENHANCED ERROR REPORT ===\n")
					f.write("Generated by OpenAI. Review suggestions before applying.\n\n")
					f.write(enhanced)
				print("LLM suggestions written to: error_llm_suggestions.txt")
			else:
				print("LLM enhancement unavailable — check OPENAI_API_KEY.")
		except Exception as e:
			print(f"LLM enhancement failed (non-fatal): {e}")
