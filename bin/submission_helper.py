...
def _retry(max_attempts: int = 3, initial_delay: float = 5, backoff: float = 2):
...
def sendemail(sample_id, config_dict, mode, submission_dir, dry_run=True):
	...
	try:
		...
		logging.error("Unable to send mail automatically. If unable to email, submission can be made manually using the sqn file.")
		logging.error("sqn_file: " + os.path.join(submission_dir, f"{sample_id}.sqn"))
		logging.error(str(e))
...
def parse_and_save_reports(reports_fetched, outdir, batch_id, use_llm=False, llm_model='gpt-4o-mini'):
	...
	if use_llm:
		try:
			from llm_helper import interpret_ncbi_report
			csv_content = all_reports.to_csv(index=False)
			interpretation = interpret_ncbi_report(csv_content, model=llm_model)
			if interpretation:
				interp_path = os.path.join(outdir, f"{batch_id}_llm_interpretation.txt")
				with open(interp_path, 'w') as f:
					f.write("=== LLM NCBI REPORT INTERPRETATION ===\n")
					f.write("Generated by OpenAI. Review before acting on suggestions.\n\n")
					f.write(interpretation)
				logging.info(f"LLM report interpretation saved to: {interp_path}")
			else:
				logging.info("LLM report interpretation unavailable — check OPENAI_API_KEY.")
		except Exception as e:
			logging.warning(f"LLM report interpretation failed (non-fatal): {e}")
...
class SubmissionConfigParser:
	...
	def load_config(self):
		...
		if not isinstance(config_dict, dict):
			logging.error("Config file is incorrect. File must have a valid yaml format.")
			sys.exit(1)
		# Fall back to environment variables for credentials (supports Nextflow Secrets and .env)
		env_mapping = {
			'NCBI_username': 'NCBI_USERNAME',
			'NCBI_password': 'NCBI_PASSWORD',
			'NCBI_ftp_host': 'NCBI_FTP_HOST',
			'NCBI_sftp_host': 'NCBI_SFTP_HOST',
		}
		for yaml_key, env_var in env_mapping.items():
			if not config_dict.get(yaml_key):
				env_val = os.environ.get(env_var)
				if env_val:
					config_dict[yaml_key] = env_val
					logging.info(f"Loaded {yaml_key} from environment variable {env_var}")
			...
			for k, v in config_dict.items():
				if self.parameters.get("gisaid", False):
					if k.startswith('GISAID') and not v:
						logging.error("Missing GISAID values in config file.")
						sys.exit(1)
				else:
					if k.startswith('NCBI') and not v:
						logging.error("Missing NCBI values in config file.")
						sys.exit(1)
		return config_dict
...
class SFTPClient:
	def __init__(self, config):
		...
		self.sftp = None
		self.ssh = None
	@_retry(max_attempts=3, initial_delay=5)
	def connect(self):
		try:
...
class FTPClient:
	...
	_BINARY_EXTENSIONS = {
		'.fasta', '.fastq', '.fq', '.fna', '.fsa', '.gff', '.gff3',
		'.gz', '.xml', '.sqn', '.sbt', '.cmt', '.ready', '.zip',
	}
	@_retry(max_attempts=3, initial_delay=5)
	def upload_file(self, file_path, destination_path):
		try:
			...
			if ext in self._BINARY_EXTENSIONS:
				with open(file_path, 'rb') as fh:
					...
					self.ftp.storbinary(f'STOR {destination_path}', fh)
			else:
				with open(file_path, 'r') as fh:
					logging.info(f"Uploading text file: {file_path}")
					self.ftp.storlines(f'STOR {destination_path}', fh)
			logging.info(f"Uploaded {file_path} to {destination_path}")
		except Exception as e:
			raise IOError(f"Failed to upload {file_path}: {e}")
	def close(self):
...
class GenbankSubmission(XMLSubmission, Submission):
	def __init__(self, parameters, submission_config, metadata_df, outdir, submission_mode, submission_dir, type, samples, sample, accession_id = None, identifier = None):
		...
		logging.debug(f"Top metadata: {self.top_metadata}")
		logging.debug(f"Biosample metadata: {self.biosample_metadata}")
		logging.debug(f"GenBank metadata: {self.genbank_metadata}")
