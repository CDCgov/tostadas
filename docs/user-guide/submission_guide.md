...
- **`new_field_name`** -- TOSTADAS will replace the field name in your metadata Excel file with this value. (Example application: you get weekly metadata Excel files and they specify 'animal\_environment' but NCBI expects 'animal\_env'; you can specify this once in the JSON file and it will be changed on every run.)
...
- **SARS-CoV-2.wwsurv.1.0**
    ...
    3. Run as:
        ...
        ```bash
        nextflow run main.nf -profile nwss,<docker|singularity> --meta_path <path/to/metadata_file.xlsx> --submission_config <path/to/submission_config.yaml>
        ```
...
- **OneHealthEnteric.1.0**
    ...
    3. Run as:
        ...
        ```bash
        nextflow run main.nf -profile pulsenet,<docker|singularity> --meta_path <path/to/metadata_file.xlsx> --submission_config <path/to/submission_config.yaml>
        ```
