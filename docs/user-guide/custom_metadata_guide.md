...
- **Value** -- Array consisting of sub-keys/values
...
- **Samples** (`samples`)
    ...
    - Will accept a single string or a list of strings for either option. Here are few acceptable variations:
        ...
        "samples": "All"
        "samples": ["All"]
        "samples": ["FL0000", "FL0001", "FL0002"]
        "samples": "FL0000"
        ```

- **Replace Empty Values** (`replace_empty_with`)
...
- `--validate_custom_fields` -- Toggles custom metadata field checks on/off. Must be set to **True** if custom field checks are wanted.
- `--custom_fields_file` -- Path to your JSON file containing custom field names and check/transformation properties for each.

!!! note
    The default value for `--validate_custom_fields` is **False** in the test profile, therefore this must be changed to **True** if doing a test run.
...
After running metadata validation, with the appropriate Nextflow parameters and your JSON file, there is a .txt log file that is generated as an output named **custom_fields_error.txt**.

This .txt log file contains information about two aspects generally:

- The actual contents within the provided JSON file:
...
- For each sample, the outcome of performing a custom field check on it (if mentioned in any under "samples"):
...
The custom_fields_error.txt file will be outputted under the **errors** directory, which is nested within the validation outputs directory.
...
- If the "samples" key is empty for a custom metadata field, or none of the provided values are strings (if some strings are present, then it will proceed with only those), then it will check **ALL** samples within the batch by default.
...
- Custom field name is empty
- There is at least one valid sample name provided (non-empty string) AND none of the sample(s) listed are in the metadata sheet
