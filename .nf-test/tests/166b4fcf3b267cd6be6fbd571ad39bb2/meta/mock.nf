...
include { RUN_VADR } from '/scicomp/home-pure/rjd0/tostadas/subworkflows/local/vadr.nf'
...
workflow {
    ...
    if (RUN_VADR.output){
        ...
        for (def name in RUN_VADR.out.getNames()) {
            serializeChannel(name, RUN_VADR.out.getProperty(name), jsonOutput)
        }	  
    
        // consumes all unnamed output channels and stores items in a json file
        def array = RUN_VADR.out as Object[]
